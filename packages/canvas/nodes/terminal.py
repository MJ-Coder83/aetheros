...
class TerminalNode:
    ...
    def resize_widget(self, widget_id: str, width: int, height: int) -> TUIWidget:
        ...
        for wid, existing in self._widgets.items():
            if wid != widget_id and updated.overlaps(existing):
                raise WidgetOverlapError(
                    f"Widget '{widget_id}' resized overlaps '{wid}'"
                )
        self._widgets[widget_id] = updated
        return updated
    ...
    def _gen_textual(self) -> str:
        ...
        if not self.widgets:
            ...
            for widget in self.widgets:
                ...
                lines.append(
                    f'        yield {textual_cls}({label_arg}, id="{widget.widget_id}")'
                )
        lines += ["", "", "if __name__ == '__main__':", f"    {self._class_name(self.label)}App().run()"]
        return "\n".join(lines)
    ...
    def _gen_rich(self) -> str:
        ...
        if not self.widgets:
            ...
            for widget in self.widgets:
                lines.append(
                    f'layout.add_split(Layout(name="{widget.widget_id}", '
                    f'size={widget.height}))'
                )
                lines.append(
                    f'layout["{widget.widget_id}"].update(Panel("{widget.label}"))'
                )
        lines += ["", "with Live(layout, console=console, screen=True):", "    input('Press Enter to exit...')"]
        return "\n".join(lines)
    ...
    def _gen_ratatui(self) -> str:
        ...
        if self.widgets:
            constraints = ", ".join(
                f"Constraint::Length({w.height})" for w in self.widgets
            )
            lines.append(
    ...
    def _heuristic_suggest(self, prompt: str) -> LayoutSuggestion:
        ...
        if True:  # always add a header
            widgets.append(TUIWidget(
                ...
                widget_type=TUIWidgetType.HEADER,
                label=self.label,
                col=0, row=0, width=self.cols, height=3,
            ))
        ...
        if "table" in q or "data" in q or "list" in q:
            widgets.append(TUIWidget(
                ...
                label="Data",
                col=0, row=body_row, width=self.cols, height=body_height,
            ))
            ...
            widgets.append(TUIWidget(
                ...
                label="Navigation",
                col=0, row=body_row, width=sidebar_w, height=body_height,
            ))
            widgets.append(TUIWidget(
                widget_id="main_panel",
                widget_type=TUIWidgetType.PANEL,
                label="Main",
                col=sidebar_w, row=body_row,
                width=self.cols - sidebar_w, height=body_height,
            ))
            ...
            widgets.append(TUIWidget(
                ...
                widget_type=TUIWidgetType.PANEL,
                label="Main",
                col=0, row=body_row, width=self.cols, height=panel_h,
            ))
            widgets.append(TUIWidget(
                widget_id="log_view",
                widget_type=TUIWidgetType.LOG,
                label="Log",
                col=0, row=body_row + panel_h,
                width=self.cols, height=body_height - panel_h,
            ))
            ...
            widgets.append(TUIWidget(
                ...
                widget_type=TUIWidgetType.PANEL,
                label="Form",
                col=0, row=body_row, width=self.cols, height=body_height - 4,
            ))
            widgets.append(TUIWidget(
                widget_id="input_field",
                widget_type=TUIWidgetType.INPUT,
                label="Input",
                col=2, row=body_row + 1, width=self.cols - 4, height=3,
            ))
        ...
        if statusbar_row > body_row:
            widgets.append(TUIWidget(
                ...
                label="Ready",
                col=0, row=statusbar_row, width=self.cols, height=3,
            ))
